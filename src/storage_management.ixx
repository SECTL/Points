...
export namespace points
{
	...
	consteval int Record::disk_size() { return get_record_field_size(); }
	...
	consteval int rule_disk_size()
	{
		return std::accumulate(std::begin(rule_field_sizes), std::end(rule_field_sizes), 0);
	}
#endif
	static_assert(rule_disk_size() == 77, "rule disk size");

	consteval int RuleRecord::disk_size() { return rule_disk_size(); }

	// 礼物 = 规则同构（desc + delta），独立类型以让 FileStore 各自持有单例
	struct GiftRecord : RuleRecord
	{
		static consteval int disk_size() { return rule_disk_size(); }
	};
	...
	public:
		...
		void load() {
			if (!std::filesystem::exists(file_path_)) {   // 全新表：空库
				pool_.clear();
				capacity_ = 0;
				alive_ = 0;
				free_head_ = FREE_NIL;
			...
			if (h.header_size != file_header_size || h.record_size != static_cast<uint32_t>(T::disk_size()))
		...
		void save() {
			...
			std::string buf;
			buf.reserve(file_header_size + static_cast<size_t>(capacity_) * T::disk_size());
